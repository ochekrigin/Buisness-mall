		...
		function Tooltip() {
			...
			this.init = function() {
				...
				this.close.on('click touchend', function(e) {
					...
					s.hide();
					$('.mapplic-list-location a').removeClass('active');
				});
			...
			this.search = function(keyword) {
				...
				$('.mapplic-list li', self.el).each(function() {
					if ($(this).text().search(new RegExp(keyword, "i")) < 0) {
						...
						$(this).addClass('mapplic-opened');
						$(this).show();
					}
				});
				if (keyword.length < 1) {
					$('.mapplic-list-category').removeClass('mapplic-opened');
				}
			...
			$.each(self.data.levels, function(index, layer) {
				...
				$.each(layer.locations, function(index, location) {
					if (location.id == id) {
						...
						var items = $('.mapplic-list-location a');
						for (var i = 0; i < items.length; i++) {
							if ($(items[i]).find('h4').text() === location.title) {
								items.removeClass('active');
								$(items[i]).addClass('active');
								$(items[i]).closest('.mapplic-list-category').addClass('mapplic-opened')
							}
						}
						// redirecting
